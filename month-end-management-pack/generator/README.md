...
- Direct cost per client is its service line's rate plus a per-client margin offset, with a revenue-weighted correction that preserves each line's total direct cost. So client gross margins genuinely vary (~27% to ~44%) while the P&L anchors are untouched; Nyle Distribution and Oakline Foods are the low-margin watch clients.
